...
// @desc     Create a post
// @access   Private. so  need token. will make that happen by auth middleware.
...
router.put("/unlike/:id", auth, async (req, res) => {
  try {
    ...
    // Check if the post has already been liked   //Check if the post has already been liked by the user. higher order array method.
    // compare the current user to the user that's logged in.
    // >0 already a like in there that has the user.
    if (
    ...
    // Get remove index.     // to get the like to remove.
...
router.delete("/comment/:id/:comment_id", auth, async (req, res) => {
  try {
    ...
    // Check user.     // make sure user that deletes the comment is the user who made the comment.
